...
/**
 * Super simple interface for describing streaming capabilities. Used in API description. You might
 * end up with gluing this with akka-streams or FS2 in attempt to gain more power and many more
 * combinators
 */
trait Stream_1_1_0[F[_], A] {

  /**
   * Takes initial `L` and combines with all `A` until `Right[R]` is returned by `f`.
   *
   * The stream calls `f` over and over with a new `A` value until either `f` returns `Right[R]` or
   * there are no more `A` left in a stream. Each consecutive call to `f` will use `l` returned by a
   * previous call to `f`.
   *
   * @return
   *   `Right[R]` if `f` returned `Right[R]` or `Left[L]` if it never did, and there are no more `A`
   *   values left.
   */
  def foldWhileM[L, R](l: L)(f: (L, A) => F[Either[L, R]]): F[Either[L, R]]
}

object Stream_1_1_0 { self =>

  def apply[F[_]](
    implicit
    F: Monad[F],
  ): Builders[F] = new Builders[F](F)

  implicit def monadStream[F[_]]: Monad[Stream_1_1_0[F, _]] = new StackSafeMonad[Stream_1_1_0[F, _]] {

    def flatMap[A, B](fa: Stream_1_1_0[F, A])(f: A => Stream_1_1_0[F, B]) = fa.flatMap(f)

    def pure[A](a: A) = single[F, A](a)

    override def map[A, B](fa: Stream_1_1_0[F, A])(f: A => B) = fa.map(f)
  }

  implicit def monoidStream[F[_]: Monad, A]: Monoid[Stream_1_1_0[F, A]] = new Monoid[Stream_1_1_0[F, A]] {

    def empty = Stream_1_1_0.empty

    def combine(x: Stream_1_1_0[F, A], y: Stream_1_1_0[F, A]) = x concat y
  }
  ...
  def lift[F[_], A](
    ...
    monad: FlatMap[F],
  ): Stream_1_1_0[F, A] = new Stream_1_1_0[F, A] {
   ...
   *
   * @see
   *   [[Builders#repeat]] for a more convenient syntax.
   */
  def repeat[F[_], A](
    fa: F[A],
  )(implicit
   ...
   *
   * @see
   *   [[Builders#single]] for a more convenient syntax.
   */
  def single[F[_], A](a: A): Stream_1_1_0[F, A] = new Stream_1_1_0[F, A] {
   ...
   *
   * It could be useful to add an error handling or retry mechanics to an existing stream.
   *
   ...
   *
   * @see
   *   [[https://github.com/evolution-gaming/retry]] library for an out of the box retry builders to
   *   use as a parameter for this function.
   */
  def around[F[_]](f: F ~> F): Stream_1_1_0[F, Unit] = new Stream_1_1_0[F, Unit] {
  ...
  def fromIterator[F[_]: Sync, A](iterator: F[Iterator[A]]): Stream_1_1_0[F, A] = {
    for {
      as <- Stream_1_1_0.lift(iterator)
      a = Sync[F].delay { if (as.hasNext) as.next().some else none[A] }
      a <- whileSome(a)
    } yield a
  }

  def whileSome[F[_]: Monad, A](a: F[Option[A]]): Stream_1_1_0[F, A] = new Stream_1_1_0[F, A] {
  ...
  def untilNone[F[_]: Monad, A](a: F[Option[A]]): Stream_1_1_0[F, A] = whileSome(a)
  ...
  final class Builders[F[_]](val F: Monad[F]) extends AnyVal {
    ...
    def apply[G[_], A](
      ...
      G: FoldWhile[G],
    ): Stream_1_1_0[F, A] = from[F, G, A](ga)(G, F)
    ...
    def empty[A]: Stream_1_1_0[F, A] = Stream_1_1_0.empty(F)

    def single[A](a: A): Stream_1_1_0[F, A] = Stream_1_1_0.single[F, A](a)

    def many[A](a: A, as: A*): Stream_1_1_0[F, A] = apply[List, A](a :: as.toList)

    def repeat[A](
      fa: F[A],
    )(implicit
      F: Monad[F],
    ): Stream_1_1_0[F, A] = self.repeat(fa)
  }
  ...
  implicit class StreamOps[F[_], A](val self: Stream_1_1_0[F, A]) extends AnyVal {

    def mapK[G[_]](to: F ~> G, from: G ~> F): Stream_1_1_0[G, A] = new Stream_1_1_0[G, A] {
    ...
    def map[B](f: A => B): Stream_1_1_0[F, B] = new Stream_1_1_0[F, B] {
    ...
    def mapM[B](
      ...
      F: FlatMap[F],
    ): Stream_1_1_0[F, B] = new Stream_1_1_0[F, B] {
    ...
    def flatMap[B](f: A => Stream_1_1_0[F, B]): Stream_1_1_0[F, B] = new Stream_1_1_0[F, B] {
    ...
    def withFilter(
      ...
      F: Applicative[F],
    ): Stream_1_1_0[F, A] = filter(f)

    def concat[B >: A](
      stream: Stream_1_1_0[F, B],
    )(implicit
      F: Monad[F],
    ): Stream_1_1_0[F, B] = new Stream_1_1_0[F, B] {
    ...
    def handleErrorWith[E](
      f: E => Stream_1_1_0[F, A],
    )(implicit
    ...
    /**
     * Similar to [[flatMap]], but allows keeping a state or shortcut the computation.
     *
     ...
     *   The initial state.
     * @param f
     *   Converts previous state and a new input element, to a new state and a stream of output
     *   elements. The stream is finished if there are no more elements in original stream, or if
     *   this function returns `None` as a state.
     *
     * @return
    ...
    def stateful[S, B](
      ...
      f: (S, A) => (Option[S], Stream_1_1_0[F, B]),
    )(implicit
      F: Functor[F],
    ): Stream_1_1_0[F, B] = new Stream_1_1_0[F, B] {
    ...
    def statefulM[S, B](
      ...
      f: (S, A) => F[(Option[S], Stream_1_1_0[F, B])],
    )(implicit
      F: FlatMap[F],
    ): Stream_1_1_0[F, B] = new Stream_1_1_0[F, B] {
     ...
     *
     * @param f
     *   Converts a new input element, to `continue` flag and a stream of output elements. The
     *   stream is finished if there are no more elements in original stream, or if this function
     *   returns (`false`, _) as a resulting tuple.
     *
     * @return
     *   Flattened stream of elements returned by `f`.
     *
     * @see
     *   [[stateful]] if stateful processing, in addition to shortcut semantics, is required.
     */
    def stateless[B](
      f: A => (Boolean, Stream_1_1_0[F, B]),
    )(implicit
      F: Functor[F],
    ): Stream_1_1_0[F, B] = new Stream_1_1_0[F, B] {
     ...
     */
    def statelessM[B](
      f: A => F[(Boolean, Stream_1_1_0[F, B])],
    )(implicit
      F: FlatMap[F],
    ): Stream_1_1_0[F, B] = new Stream_1_1_0[F, B] {
    ...
    def foldLeftM[B](
      ...
      statefulM(b) { (b, a) =>
        f(b, a).map { b =>
          (b.some, Stream_1_1_0.single[F, B](b))
        }
      }
    }
     ...
     *
     * Similar to [[concat]], but allows to use the last element emitted by a first stream to
     ...
     *
     * @param f
     *   Converts a last stream element, to a stream of output elements to be emited after all
     *   elements of original stream are emitted. If the original stream had no elements then `None`
     *   is passed to `f` instead.
     *
     * @return
     *   Stream the same elements as original stream does and then stream the elements returned by
     *   `f`.
     */
    def flatMapLast[B >: A](
      f: Option[A] => Stream_1_1_0[F, B],
    )(implicit
      F: Monad[F],
    ): Stream_1_1_0[F, B] = new Stream_1_1_0[F, B] {
     ...
     *
     * Similar to [[flatMapLast]], but also applies itself to the produced elements until `None` is
     * returned by `f`.
     *
     ...
     * @param f
     *   Converts a last stream element, to a stream of output elements to be emited after all
     *   elements of original stream are emitted. Also applies the same function to the new last
     *   element until the function returns `None`.
     *
     * @return
     *   Stream the same elements as original stream does, then stream the elements returned by
     *   recursive application of `f` to last element and last elements of the streams produced by
     *   itself.
     */
    def chain(
      f: A => Option[Stream_1_1_0[F, A]],
    )(implicit
      F: Monad[F],
    ): Stream_1_1_0[F, A] = {
      chainM { a => f(a).pure[F] }
    ...
    def chainM(
      ...
      def foldWhileM[L, R](l: L)(f1: (L, A) => F[Either[L, R]]) = {
        ...
        def drain(stream: Stream_1_1_0[F, A], l: L) = {
  ...
  implicit class FlattenOps[F[_], A](val self: Stream_1_1_0[F, Stream_1_1_0[F, A]]) extends AnyVal {

    def flatten: Stream_1_1_0[F, A] = self.flatMap(identity)
  }
}
